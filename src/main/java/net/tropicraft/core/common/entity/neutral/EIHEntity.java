...
//        goalSelector.addGoal(8, new LookRandomlyGoal(this));
    ...
    public boolean hurt(final DamageSource source, final float amount) {
        ...
        if (source.getDirectEntity() instanceof Player) {
            ...
            //May not be correct
            if (!heldItem.isEmpty() && heldItem.getItem() instanceof PickaxeItem pickaxeItem && pickaxeItem.getTier().getLevel() >= 1){//heldItem.getItem().getHarvestLevel(heldItem, ToolType.PICKAXE, player, null) >= 1) {
                return super.hurt(source, amount);
            } else {
