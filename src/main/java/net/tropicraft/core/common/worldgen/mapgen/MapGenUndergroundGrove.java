...
import net.tropicraft.core.common.block.BlockTikiTorch.TorchSection;
...
public class MapGenUndergroundGrove {
	...
	public ChunkPrimer generate(int x, int z, ChunkPrimer primer) {
		BlockPos groveCoords = getGroveNear(worldObj, x, z);
		
		if (groveCoords != null) {
			centerX = groveCoords.getX();
			y = groveCoords.getY();
			centerZ = groveCoords.getZ();
			x *= 16;
			z *= 16;
		} else {
			return primer;
    		...
    		for(int k = 0; k < 16; k++) {
    			...
    			for(double j = -height; j < height; j++) {
    				if((relativeX / length) + ((j * j) / height) + (relativeZ / width) <= 1) {
    					placeBlock(i, y + (int)j, k, Blocks.AIR, primer);
    				}
    			}
    			...
    			if ((i + x) % 16 == 0 && (k + z) % 16 == 0) {
    				...
    				if (getBlock(i, y - j, k, primer) == Blocks.AIR && getBlock(i, y - j + 1, k, primer) == Blocks.AIR && getBlock(i, y - j + 2, k, primer) == Blocks.AIR && rand.nextInt(3) != 0) {
    					placeBlockAndMeta(i, y - j, k, BlockRegistry.tikiTorch.getDefaultState().withProperty(BlockTikiTorch.SECTION, TorchSection.LOWER), primer);
    					placeBlockAndMeta(i, y - j + 1, k, BlockRegistry.tikiTorch.getDefaultState().withProperty(BlockTikiTorch.SECTION, TorchSection.MIDDLE), primer);
    					placeBlockAndMeta(i, y - j + 2, k, BlockRegistry.tikiTorch.getDefaultState().withProperty(BlockTikiTorch.SECTION, TorchSection.UPPER), primer);
    				}
    			}
    		}
    	}
     ...
     */
    public BlockPos getGroveNear(World worldObj, int i, int j) {
        ...
        for(int x = i - range; x <= i + range; x++) {
            for(int z = j - range; z <= j + range; z++) {
                ...
                if (canGenGroveAtCoords(worldObj, x, z)) {
                    return new BlockPos(x * 16 + 8, rand.nextInt(5) + 20, z * 16 + 8);
                }
            }
        }
	...
	private void placeBlock(int x, int y, int z, Block block, ChunkPrimer primer) {
	    primer.setBlockState(x, y, z, block.getDefaultState());
	}
	
	private void placeBlockAndMeta(int x, int y, int z, IBlockState state, ChunkPrimer primer) {
	    primer.setBlockState(x, y, z, state);
	}
	
	private Block getBlock(int x, int y, int z, ChunkPrimer primer) {
		return primer.getBlockState(x, y, z).getBlock();
	}
}
