...
public class MapGenTropicsCaves extends MapGenBase {
	...
	public void generate(World world, int x, int z, ChunkPrimer chunkPrimer)
    {
        ...
        for (int j1 = x - range; j1 <= x + range; ++j1)
        {
            for (int k1 = z - range; k1 <= z + range; ++k1)
            {
                ...
                this.recursiveGenerate(world, j1, k1, x, z, chunkPrimer);
            }
        }
    }
    ...
    protected boolean isOceanBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ) {
        ...
        return block == Blocks.FLOWING_WATER || block == Blocks.WATER || block == BlockRegistry.tropicsWater;
    }
    ...
    protected boolean canReplaceBlock(IBlockState state1, IBlockState state2) {
        return state1.getBlock() == Blocks.STONE ? true : (state1.getBlock() == Blocks.DIRT ? true : (state1.getBlock() == Blocks.GRASS ? true : (state1.getBlock() == Blocks.HARDENED_CLAY ? true : (state1.getBlock() == Blocks.STAINED_HARDENED_CLAY ? true : (state1.getBlock() == Blocks.SANDSTONE ? true : (state1.getBlock() == Blocks.RED_SANDSTONE ? true : (state1.getBlock() == Blocks.MYCELIUM ? true : (state1.getBlock() == Blocks.SNOW_LAYER ? true : (state1.getBlock() == Blocks.SAND || state1.getBlock() == Blocks.GRAVEL) && state2.getMaterial() != Material.WATER))))))));
    }
