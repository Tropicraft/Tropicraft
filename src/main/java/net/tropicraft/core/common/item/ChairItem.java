...
public class ChairItem extends Item implements IColoredItem {

    private final DyeColor color;

    public ChairItem(final Properties properties, final DyeColor color) {
        super(properties);
        this.color = color;
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity placer, Hand hand) {
        ...
        if (rayTraceResult.getType() == net.minecraft.util.math.RayTraceResult.Type.MISS) {
            ...
            if (rayTraceResult.getType() == net.minecraft.util.math.RayTraceResult.Type.BLOCK) {
                ...
                final ChairEntity chair = TropicraftEntities.CHAIR.get().create(world);
                chair.moveToBlockPosAndAngles(new BlockPos(hitVec.x, hitVec.y, hitVec.z), 0, 0);
                chair.setMotion(Vec3d.ZERO);
                chair.rotationYaw = placer.rotationYaw + 180;
                chair.setColor(this.color);

                if (!world.isCollisionBoxesEmpty(chair, chair.getBoundingBox().grow(-0.1D))) {
