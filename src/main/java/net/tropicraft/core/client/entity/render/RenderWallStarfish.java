...
public class RenderWallStarfish extends Render<EntityWallStarfish> {
	...
	@Override
	public void doRender(EntityWallStarfish entity, double x, double y, double z, float yaw, float partialTicks) {
		...
		float offsetX = (float)(wallStarfish.posX - x);
		float offsetY = (float)(wallStarfish.posY - y);
		float offsetZ = (float)(wallStarfish.posZ - z);
		GlStateManager.translate(offsetX, offsetY, offsetZ);
		GlStateManager.rotate(wallStarfish.rotationYaw, 0.0F, 1.0F, 0.0F);
		...
		for (int layer = 0; layer < type.getLayerCount(); layer++) {
			...
			GlStateManager.translate(0f, 0f, -type.getLayerHeights()[layer]);
		}
	...
	@Override
	protected ResourceLocation getEntityTexture(EntityWallStarfish entity) {
		return null;
	}
}