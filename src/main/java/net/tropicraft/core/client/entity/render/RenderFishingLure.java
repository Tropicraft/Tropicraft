...
public class RenderFishingLure extends Render<EntityFishHook>
{
    ...
    public void doRender(EntityFishHook entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        ...
        if (entity.angler != null && !this.renderOutlines)
        {
            ...
            //first person
            if (false/* && (this.renderManager.options == null || this.renderManager.options.thirdPersonView <= 0) && entity.angler == Minecraft.getMinecraft().player*/)
            {
            ...
            for (int i1 = 0; i1 <= 16; ++i1)
            {
                ...
                vertexbuffer.pos(x + d10 * (double)f10, y + d11 * (double)(f10 * f10 + f10) * 0.5D + 0.25D, z + d12 * (double)f10)
                        .color(170, 170, 170, 255).endVertex();
            }
