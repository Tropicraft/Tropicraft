...
public class TropicraftRenderUtils {
    ...
    public static void renderItem(ItemStack itemStack, final float scale, boolean leftHand, PoseStack stack, MultiBufferSource buffer, int combinedLightIn, int combinedOverlayIn, BakedModel modelIn, int id) {
        if (!itemStack.isEmpty()) {
            ...
            Minecraft.getInstance().getItemRenderer().renderStatic(itemStack, ItemTransforms.TransformType.FIXED, combinedLightIn, combinedOverlayIn, stack, buffer, id);
            stack.popPose();
        }
    }
}
